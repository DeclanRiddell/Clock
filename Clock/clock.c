...
void process_key(keybits KeyCode)
{
    ...
    if ( ( KeyCode & 0xff00 ) == 0 ) {  // no ASCII code, so mouse hit
        ...
        if (KeyRow == 0) {
            switch (KeyCol) {
                case 0: // 24-hour mode
                    ...
                    view_props &= 0xfe; // switch off ampm bit
                    set_view_properties(view_props);
                    break;
                case 1: // AM/PM mode
                    view_props = get_view_properties();
                    view_props |= 0x01; // switch on ampm bit
                    set_view_properties(view_props);
                    break;
                case 2: // date
                    date_mode_end = time( (time_t *) NULL) + 5;
                    // turn on date mode
                    view_props = get_view_properties();
                    view_props |= DATE_MODE; // switch on ampm bit
                    set_view_properties(view_props);
                    break;
                case 3: // test
                     test_mode_end = time( (time_t *) NULL) + 5;
                     view_props = get_view_properties();
                     view_props |= TEST_MODE; // switch on ampm bit
                     set_view_properties(view_props);
                     break;
                case 4: // off
                    stop_clock();
        ...
        } else if (KeyRow == 1) { // optional buttons
            switch (KeyCol) {
                case 0:
                    break;
            }
